...
presale_w_address = "0x08F6372770f2717b04188af6272fc824fd98C344"
...
async function BuyFunc() {
    ...
    const sttabi = [{ inputs: [], name: "buy tokens", outputs: [{ internalType: "bool", name: "", type: "bool" }], stateMutability: "payable", type: "function" }];
...
async function BuyButton() {
    let bnbVal = document.getElementById("presaleAmount").value.replace(/,/, ".");
    if (bnbVal == 0 || bnbVal < 0.01) alert("Contribution value must be greater than or equal to 0.1");
    else {
        if (provider == null) {
            await Connect();
